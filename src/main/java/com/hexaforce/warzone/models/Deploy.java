...
public class Deploy implements Order, Serializable {
        ...
        if (validate(p_gameContext)) {
            for (Country l_country : p_gameContext.getD_map().getD_countries()) {
                if (l_country.getD_countryName().equalsIgnoreCase(this.d_targetCountryName)) {
                    Integer l_armiesToUpdate = (l_country.getD_armies() == null)
                            ? this.d_numberOfArmiesToDeploy
                            : l_country.getD_armies() + this.d_numberOfArmiesToDeploy;
                    l_country.setD_armies(l_armiesToUpdate);
                    setD_orderExecutionLog(
                            l_armiesToUpdate
                                    + " armies have been successfully deployed to country: "
            ...
            setD_orderExecutionLog(
                    "Deploy Order '"
                            ...
                            + this.d_targetCountryName
                            + "' specified in the deploy command does not belong to the player: "
    ...
    public boolean validate(GameContext p_gameContext) {
        Country l_country = d_playerInitiator.getD_countriesOwned().stream()
                .filter(
                        l_pl -> l_pl.getD_countryName().equalsIgnoreCase(this.d_targetCountryName.toString()))
                .findFirst()
                .orElse(null);
