...
public class OrderExecutionPhase extends Phase {
  ...
  public void onPhaseInitialization(boolean isTournamentMode) {
    ...
    if (this.checkEndOftheGame(d_gameContext)) return;
  ...
  protected void validateAssignCountries(
      Command command, Player player, boolean isTournamentMode, GameContext p_gameContext)
      throws InvalidCommand, InvalidMap, IOException {
    printInvalidCommandInCurrentState();
  }
  ...
  /**
   * Checks if a single player has conquered all countries of the map to indicate the end of the
   * game.
   *
