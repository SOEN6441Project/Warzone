...
/**
 * 
 * This class represents the implementation of a Airlift card order. The Airlift
 * Card allows you to transfer your armies long distances. Each time you play
 * one, you can do a single transfer from any of your territories to any other
 * territory of yours.
 */
public class Airlift implements Card {
     ...
     *
     * @param p_sourceCountryName The source country from which armies are to be
     *                            airlifted.
     * @param p_targetCountryName The country where the airlifted armies will be
     *                            placed.
    ...
    public void execute(GameContext p_gameContext) {
        if (validate(p_gameContext)) {
            ...
            this.setD_orderExecutionLog("Cannot Complete Execution of the provided Airlift Command!", "error");
            p_gameContext.updateLog(d_orderExecutionLog, "effect");
        }
    }
    ...
    public boolean validate(GameContext p_gameContext) {
        ...
        if (this.d_numberOfArmies > l_sourceCountry.getD_armies()) {
            this.setD_orderExecutionLog(this.currentOrder()
                    + " is not executed as the number of armies specified in the card order exceeds the armies of the source country : "
