...
public class Bomb implements Card {
    ...
    /**
     * Validates whether the target country belongs to the player who executed the
     * order or not.
     * 
     * @param p_gameContext The current state of the game
    ...
    public boolean validate(GameContext p_gameContext) {
        ...
        if (!CommonUtil.isNull(l_country)) {
            this.setD_orderExecutionLog(this.currentOrder() + " is not executed since Target country : "
                    + this.d_targetCountryID + " given in the bomb command is owned by the player : "
    ...
    public Boolean checkValidOrder(GameContext p_gameContext) {
        ...
        if (l_targetCountry == null) {
            this.setD_orderExecutionLog("Invalidate Target Country! It doesn't exist on the map!", "error");
            return false;
        }
        return true;
    }
