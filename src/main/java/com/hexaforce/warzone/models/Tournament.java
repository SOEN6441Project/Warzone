...
public class Tournament implements Serializable {
     ...
     * @throws InvalidCommand Returned if the command provided is syntactically or
     *                        logically invalid
     */
    public boolean parseTournamentCommand(GameContext p_gameContext, String p_operation, String p_argument,
            WarzoneEngine p_gameEngine) throws InvalidMap, InvalidCommand {
    ...
    private boolean parseNoOfTurnsArguments(String p_argument, WarzoneEngine p_gameEngine) {
        ...
        if (maxTurns >= 10 && maxTurns <= 50) {
            ...
            p_gameEngine.setD_gameEngineLog(
                    "User entered an invalid number of turns in the command. The range of turns is 10 <= number of turns <= 50",
                    "effect");
            return false;
        }
    }
     ...
     * @throws InvalidMap Returned if the map given in the command is invalid
     */
    private boolean parseNoOfGameArgument(String p_argument, WarzoneEngine p_gameEngine) throws InvalidMap {
        ...
        if (noOfGames >= 1 && noOfGames <= 5) {
            ...
            p_gameEngine.setD_gameEngineLog(
                    "User entered an invalid number of games in the command. The range of games is 1 <= number of games <= 5",
                    "effect");
            return false;
        }
    }
     ...
     * @return True if parsing of strategy information is successful; otherwise,
     *         false
     */
    private boolean parseStrategyArguments(GameContext p_gameContext, String p_argument, WarzoneEngine p_gameEngine) {
        ...
        for (String strategy : listOfPlayerStrategies) {
            ...
            if (!Constants.TOURNAMENT_PLAYER_BEHAVIORS.contains(strategy)) {
                p_gameEngine.setD_gameEngineLog(
                        "Invalid strategy passed in the command. Only Aggressive, Benevolent, Random, Cheater strategies are allowed.",
                        "effect");
                return false;
            }
        }
        if (playerStrategiesSize >= 2 && playerStrategiesSize <= 4) {
            setTournamentPlayers(p_gameEngine, listOfPlayerStrategies, p_gameContext.getD_players(),
                    playersInTheGame);
        } else {
            p_gameEngine.setD_gameEngineLog(
                    "User entered an invalid number of strategies in the command. The range of strategies is 2 <= strategy <= 4",
                    "effect");
            return false;
        }
     ...
     */
    private void setTournamentPlayers(WarzoneEngine p_gameEngine, String[] listOfPlayerStrategies,
            List<Player> listOfPlayers, List<Player> playersInTheGame) {
        for (String strategy : listOfPlayerStrategies) {
            for (Player pl : listOfPlayers) {
                if (pl.getD_playerBehaviorStrategy().getPlayerBehavior().equalsIgnoreCase(strategy)) {
                    ...
                    p_gameEngine.setD_gameEngineLog("Player:  " + pl.getPlayerName() + " with strategy: " + strategy
                            + " has been added to the tournament.", "effect");
                }
            }
        }
    }
     ...
     * @param p_gameEngine Game engine object
     * @return True if parsing of maps to the tournament object is successful;
     *         otherwise, false
     * @throws InvalidMap Returned if the map given in the command is invalid
     */
    private boolean parseMapArguments(String p_argument, WarzoneEngine p_gameEngine) throws InvalidMap {
        ...
        if (mapFilesSize >= 1 & mapFilesSize <= 5) {
            ...
            p_gameEngine.setD_gameEngineLog(
                    "User entered an invalid number of maps in the command. The range of maps is 1 <= map <= 5",
                    "effect");
            return false;
        }
        return true;
    ...
    /**
     * Validates the tournament command and checks if the required information is
     * there in the command or not.
     * 
     * @param operationsList Operations list given in the command
     * @param command        Tournament command
     * @return True if the command is valid; otherwise, false
     */
    public boolean requiredTournamentArgPresent(List<java.util.Map<String, String>> operationsList, Command command) {
        String argumentKey = new String();
        if (operationsList.size() != 4)
            return false;
        ...
        if (!argumentKey.equalsIgnoreCase("MPGD"))
            return false;
        return true;
    }
