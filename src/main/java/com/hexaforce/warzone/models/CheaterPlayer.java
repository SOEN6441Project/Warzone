...
/**
 * This class represents the Cheater Player, which launches direct attacks on neighboring enemy
 * countries during the issue order phase. Additionally, it doubles the number of armies on its
 * countries that have enemy neighbors.
 */
public class CheaterPlayer extends PlayerBehaviorStrategy {
      ...
      while (p_player.getD_noOfUnallocatedArmies() > 0) {
        ...
        String logMessage =
            "Cheater Player: "
                ...
                + armiesToDeploy
                + " armies to  "
                + randomCountry.getD_countryName();
  ...
  private void doubleArmyOnEnemyNeighboredCounties(Player p_player, GameContext p_gameContext) {
    ...
    for (Country ownedCountry : countriesOwned) {
      ...
      if (countryEnemies.size() == 0) continue;

      int armiesInTerritory = ownedCountry.getD_armies();

      if (armiesInTerritory == 0) continue;
      ...
      String logMessage =
          "Cheater Player: "
              + p_player.getPlayerName()
              + " doubled the armies ( Now: "
              + armiesInTerritory * 2
              + ") in "
              + ownedCountry.getD_countryName();
   ...
   */
  private void conquerTargetCountry(
      GameContext p_gameContext, Player targetPlayer, Player cheaterPlayer, Country targetCountry) {
  ...
  private void updateContinents(
      Player cheaterPlayer, Player targetPlayer, GameContext p_gameContext) {
    ...
    playerService.performContinentAssignment(
        playesList, p_gameContext.getD_map().getD_continents());
  }
  ...
  private ArrayList<Integer> getEnemies(Player p_player, Country country) {
    ...
    for (Integer countryID : country.getD_adjacentCountryIds()) {
      if (!p_player.getCountryIDs().contains(countryID)) enemyNeighbors.add(countryID);
    }
