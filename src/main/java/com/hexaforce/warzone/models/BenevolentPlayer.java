...
/**
 * Represents a Benevolent Player class, focusing solely on defending owned countries and refraining
 * from attacking.
 */
public class BenevolentPlayer extends PlayerBehaviorStrategy {
        ...
        switch (l_index) {
          ...
          case 3:
            if (p_player.getD_cardsOwnedByPlayer().size() == 1) {
              ...
              l_command =
                  createCardOrder(
                      p_player, p_gameContext, p_player.getD_cardsOwnedByPlayer().get(0));
              break;
            } else {
  ...
  public String createAdvanceOrder(Player p_player, GameContext p_gameContext) {
    ...
    System.out.println(
        "advance "
            ...
            + " "
            + l_weakestTargetCountry.getD_countryName()
            + " "
            + l_armiesToSend);
    return "advance "
        + l_randomSourceCountry.getD_countryName()
  ...
  public String createCardOrder(Player p_player, GameContext p_gameContext, String p_cardName) {
    ...
    Country l_randomEnemyNeighbor =
        p_gameContext
            .getD_map()
            .getCountry(
                randomEnemyNeighbor(p_player, l_randomOwnCountry)
                    .get(
                        l_random.nextInt(
                            randomEnemyNeighbor(p_player, l_randomOwnCountry).size() > 0
    ...
    switch (p_cardName) {
      ...
      case "airlift":
        return "airlift "
            + l_randomOwnCountry.getD_countryName()
            + " "
  ...
  private ArrayList<Integer> randomEnemyNeighbor(Player p_player, Country p_country) {
    ...
    for (Integer l_countryID : p_country.getD_adjacentCountryIds()) {
      if (!p_player.getCountryIDs().contains(l_countryID)) l_enemyNeighbors.add(l_countryID);
    }
  ...
  private Boolean checkIfArmiesDeployed(Player p_player) {
    if (p_player.getD_countriesOwned().stream()
        .anyMatch(l_country -> l_country.getD_armies() > 0)) {
      return true;
    }
    return false;
  }
