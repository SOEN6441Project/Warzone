...
/**
 * 
 * This class represents the implementation of a Blockade order. Blockade cards
 * are used to change one of your territories into a neutral territory and
 * triple the number of armies on that territory.
 */
public class Blockade implements Card {
    ...
    /**
     * Initializes a Blockade order with the player owning the card and the target
     * country.
     *
    ...
    /**
     * Executes the Blockade order, which turns the target country into a neutral
     * territory with tripled armies.
     *
    ...
    public void execute(GameContext p_gameContext) {
        if (validate(p_gameContext)) {
            ...
            int l_noOfArmiesOnTargetCountry = l_targetCountry.getD_armies() == 0 ? 1 : l_targetCountry.getD_armies();
            l_targetCountry.setD_armies(l_noOfArmiesOnTargetCountry * 3);

            // Change territory to a neutral territory
            d_playerInitiator.getD_coutriesOwned().remove(l_targetCountry);

            Player l_playerNeutral = p_gameContext.getD_players().stream()
            ...
            if (l_playerNeutral != null) {
                l_playerNeutral.getD_coutriesOwned().add(l_targetCountry);
            } else {
            ...
            this.setD_orderExecutionLog("\nPlayer : " + this.d_playerInitiator.getPlayerName()
                    + " is executing a blockade on Country :  " + l_targetCountry.getD_countryName()
                    + " with armies :  " + l_targetCountry.getD_armies(), "default");
