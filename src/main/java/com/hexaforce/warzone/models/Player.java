...
public class Player implements Serializable {
  ...
  public Player(String p_playerName) {
    ...
    this.d_countriesOwned = new ArrayList<Country>();
  ...
  public void createDeployOrder(String p_commandEntered) {
    ...
    try {
      ...
      if (validateDeployOrderArmies(this, l_noOfArmies)) {
        ...
        Integer l_unallocatedarmies =
            this.getD_noOfUnallocatedArmies() - Integer.parseInt(l_noOfArmies);
        this.setD_noOfUnallocatedArmies(l_unallocatedarmies);
        this.setD_playerLog(
            "Deploy order has been added to queue for execution. For player: " + this.d_name,
            "log");
      }
    } catch (Exception l_e) {
   ...
   */
  public void issue_order(IssueOrderPhase p_issueOrderPhase)
      throws InvalidCommand, IOException, InvalidMap {
    p_issueOrderPhase.promptForOrders(this);
  }

  /**
   * Gives the first order in the players list of orders, then removes it from the list.
   *
   ...
   */
  public void createAdvanceOrder(String p_commandEntered, GameContext p_gameContext) {
    try {
      if (p_commandEntered.split(" ").length == 4) {
        ...
        if (this.checkCountryExists(l_sourceCountry, p_gameContext)
            && this.checkCountryExists(l_targetCountry, p_gameContext)
            && !checkZeroArmiesInOrder(l_noOfArmies)
            && checkAdjacency(p_gameContext, l_sourceCountry, l_targetCountry)) {
          this.order_list.add(
              new Advance(this, l_sourceCountry, l_targetCountry, Integer.parseInt(l_noOfArmies)));
          this.setD_playerLog(
              "Advance order has been added to queue for execution. For player: " + this.d_name,
              "log");
        }
      } else {
   ...
   * @param p_gameContext current state of the map
   * @return true if country exists in map or else false
   */
  private Boolean checkCountryExists(String p_countryName, GameContext p_gameContext) {
    if (p_gameContext.getD_map().getCountryByName(p_countryName) == null) {
      this.setD_playerLog(
          "Country : "
              + p_countryName
  ...
  public boolean checkAdjacency(
      GameContext p_gameContext, String p_sourceCountryName, String p_targetCountryName) {
    Country l_sourceCountry = p_gameContext.getD_map().getCountryByName(p_sourceCountryName);
    Country l_targetCountry = p_gameContext.getD_map().getCountryByName(p_targetCountryName);
    Integer l_targetCountryId =
        l_sourceCountry.getD_adjacentCountryIds().stream()
            .filter(l_adjCountry -> l_adjCountry == l_targetCountry.getD_countryId())
            .findFirst()
            .orElse(null);
    if (l_targetCountryId == null) {
      this.setD_playerLog(
          "Advance order cant be issued since target country : "
              + p_targetCountryName
              + " is not adjacent to source country : "
              + p_sourceCountryName,
          "error");
      return false;
    }
  ...
  /**
   * This method will assign any random card from the set of available cards to the player once he
  ...
  public boolean negotiationValidation(String p_targetCountryName) {
    ...
    for (Player p : d_negotiatedWith) {
      if (p.getCountryNames().contains(p_targetCountryName)) l_canAttack = false;
    }
    return l_canAttack;
  }
    ...
    if (checkCardArguments(p_commandEntered)) {
      switch (p_commandEntered.split(" ")[0]) {
        case "airlift":
          Card l_newOrder =
              new Airlift(
                  p_commandEntered.split(" ")[1],
                  p_commandEntered.split(" ")[2],
                  Integer.parseInt(p_commandEntered.split(" ")[3]),
                  this);
          if (l_newOrder.checkValidOrder(p_gameContext)) {
            this.order_list.add(l_newOrder);
            this.setD_playerLog("Card Command Added to Queue for Execution Successfully!", "log");
            p_gameContext.updateLog(getD_playerLog(), "effect");
          }
          break;
        case "blockade":
          Card l_blockadeOrder = new Blockade(this, p_commandEntered.split(" ")[1]);
          if (l_blockadeOrder.checkValidOrder(p_gameContext)) {
        ...
        case "bomb":
          ...
          if (l_bombOrder.checkValidOrder(p_gameContext)) {
        ...
        case "negotiate":
          ...
          if (l_negotiateOrder.checkValidOrder(p_gameContext)) {
