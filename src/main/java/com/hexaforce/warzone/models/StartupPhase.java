...
public class StartupPhase extends Phase {
    ...
    if (l_operations_list == null || l_operations_list.isEmpty()) {
      ...
      for (java.util.Map<String, String> l_map : l_operations_list) {
        if (p_command.checkRequiredKeysPresent(Constants.ARGUMENTS, l_map)) {
          d_mapService.editMap(d_gameContext, l_map.get(Constants.ARGUMENTS));
        } else {
          throw new InvalidCommand(Constants.INVALID_COMMAND_ERROR_EDITMAP);
        }
      }
      ...
      for (java.util.Map<String, String> l_map : l_operations_list) {
        if (p_command.checkRequiredKeysPresent(Constants.ARGUMENTS, l_map)) {
          boolean l_fileUpdateStatus =
              d_mapService.saveMap(d_gameContext, l_map.get(Constants.ARGUMENTS));
    ...
    if (null == l_operations_list || l_operations_list.isEmpty()) {
      ...
      for (java.util.Map<String, String> l_map : l_operations_list) {
        if (p_command.checkRequiredKeysPresent(Constants.ARGUMENTS, l_map)) {
          ...
          Map l_mapToLoad = d_mapService.loadMap(d_gameContext, l_map.get(Constants.ARGUMENTS));
          if (l_mapToLoad.validate()) {
            ...
            d_mapService.resetMap(d_gameContext, l_map.get(Constants.ARGUMENTS));
          }
          if (!l_flagValidate) {
            d_mapService.resetMap(d_gameContext, l_map.get(Constants.ARGUMENTS));
          }
  ...
  public void validateAssignCountries(Command p_command, Player p_player) throws InvalidCommand {
    ...
    if (CommonUtil.isCollectionEmpty(l_operations_list)) {
      ...
      d_playerService.assignColors(d_gameContext);
