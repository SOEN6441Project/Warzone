...
public class PlayerService {
  ...
  public List<Player> addRemovePlayers(
    ...
    boolean l_playerNameAlreadyExist =
        !isPlayerNameUnique(p_existingPlayerList, l_enteredPlayerName);
  ...
  public void assignColors(GameContext p_gameContext) {
    if (!checkPlayersAvailability(p_gameContext)) return;

    List<Player> l_players = p_gameContext.getD_players();

    for (int i = 0; i < l_players.size(); i++) {
      l_players.get(i).setD_color(COLORS.get(i));
    }
  }
    ...
    for (Player l_pl : p_players) {
      if (!l_pl.getPlayerName().equalsIgnoreCase("Neutral")) {
        ...
        for (int i = 0; i < p_countriesPerPlayer; i++) {
          ...
          if (l_pl.getD_countriesOwned() == null) l_pl.setD_countriesOwned(new ArrayList<>());
  ...
  /**
   * Checks if player is having any continent as a result of random country assignment.
   *
   * @param p_players list of all available players
      ...
      if (!CommonUtil.isCollectionEmpty(l_pl.getD_countriesOwned())) {
        ...
        for (Continent l_cont : p_continents) {
          ...
          if (l_countriesOwned.containsAll(l_countriesOfContinent)) {
            if (l_pl.getD_continentsOwned() == null) l_pl.setD_continentsOwned(new ArrayList<>());
  ...
  public Integer calculateArmiesForPlayer(Player p_player) {
    Integer l_armies =
        null != p_player.getD_noOfUnallocatedArmies() ? p_player.getD_noOfUnallocatedArmies() : 0;
   ...
   * @return boolean true if unexecuted orders exists with any of the players or else false
   */
   ...
   *
   * @param p_playersList players involved in game
   * @return boolean true if unassigned armies exists with any of the players or else false
   */
  ...
  public boolean checkForMoreOrders(List<Player> p_playersList) {
    for (Player l_player : p_playersList) {
      if (l_player.getD_moreOrders()) return true;
    }
    return false;
  }
