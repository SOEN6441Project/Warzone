...
public class GameLoadingSaving {
    ...
    public static void saveGame(Phase phase, String filename) {
        try {
            FileOutputStream gameSaveFile = new FileOutputStream(
                    Constants.SRC_MAIN_RESOURCES + "/" + filename);
     ...
     * @throws IOException            Input-output exception when the file is not
     *                                found
    ...
    public static Phase loadGame(String filename) throws IOException, ClassNotFoundException {
        ObjectInputStream inputStream = new ObjectInputStream(
                new FileInputStream(Constants.SRC_MAIN_RESOURCES + "/" + filename));
