...
/**
 * The ConquestMapFileWriter class is responsible for writing and creating
 * Conquest map files.
 */
public class ConquestMapFileWriter implements Serializable {
    ...
    public void parseMapToFile(GameContext p_gameContext, FileWriter p_writer, String p_mapFormat) throws IOException {
        ...
        if (null != p_gameContext.getD_map().getD_countries() && !p_gameContext.getD_map().getD_countries().isEmpty()) {
     ...
     */
    private void writeCountryAndBoarderMetaData(GameContext p_gameContext, FileWriter p_writer) throws IOException {
        String l_countryMetaData = new String();

        // Writes Country Objects to File And Organizes Border Data for each of them
        p_writer.write(System.lineSeparator() + Constants.CONQUEST_TERRITORIES + System.lineSeparator());
        for (Country l_country : p_gameContext.getD_map().getD_countries()) {
            ...
            l_countryMetaData = l_country.getD_countryName().concat(",dummy1,dummy2,")
                    .concat(p_gameContext.getD_map().getContinentByID(l_country.getD_continentId())
                            .getD_continentName());
