...
public class MapService implements Serializable {
    ...
    /**
     * Method is responsible for creating a new map if the map to be edited does not
     * exist, and if it exists it parses the map file to the game state object.
     *
     ...
     *                     name is invalid
     */
    public void editMap(GameContext p_gameContext, String p_editFilePath) throws IOException, InvalidMap {
        ...
        if (l_fileToBeEdited.createNewFile()) {
            ...
            p_gameContext.updateLog(p_editFilePath + " already exists and is loaded for editing", "effect");
        }
    }
     ...
     */
    public void editFunctions(GameContext p_gameContext, String p_argument, String p_operation,
            Integer p_switchParameter)
            throws IOException, InvalidMap, InvalidCommand {
        Map l_updatedMap;
        String l_mapFileName = p_gameContext.getD_map().getD_mapFile();
        Map l_mapToBeUpdated = (CommonUtil.isNull(p_gameContext.getD_map().getD_continents())
        ...
        if (!CommonUtil.isNull(l_mapToBeUpdated)) {
            switch (p_switchParameter) {
                case 1:
                    l_updatedMap = addRemoveContinents(p_gameContext, l_mapToBeUpdated, p_operation, p_argument);
                    break;
                case 2:
                    l_updatedMap = addRemoveCountry(p_gameContext, l_mapToBeUpdated, p_operation, p_argument);
                    break;
                case 3:
                    l_updatedMap = addRemoveNeighbour(p_gameContext, l_mapToBeUpdated, p_operation, p_argument);
                    break;
                default:
    ...
    /**
     * Constructs an updated Continents list based on passed operations - Add/Remove
     * and Arguments.
     *
     * @param p_gameContext    Current GameContext Object
     ...
     * @throws InvalidMap invalidmap exception
     */
    public Map addRemoveContinents(GameContext p_gameContext, Map p_mapToBeUpdated, String p_operation,
            String p_argument)
            throws InvalidMap {
        ...
        try {
            if (p_operation.equalsIgnoreCase("add") && p_argument.split(" ").length == 2) {
                ...
                this.setD_MapServiceLog("Continent " + p_argument.split(" ")[0] + " added successfully!",
                        p_gameContext);
     ...
     * @throws InvalidMap invalidmap exception
     */
    public Map addRemoveCountry(GameContext p_gameContext, Map p_mapToBeUpdated, String p_operation, String p_argument)
            throws InvalidMap {

        try {
            if (p_operation.equalsIgnoreCase("add") && p_argument.split(" ").length == 2) {
                p_mapToBeUpdated.addCountry(p_argument.split(" ")[0], p_argument.split(" ")[1]);
                this.setD_MapServiceLog("Country " + p_argument.split(" ")[0] + " added successfully!", p_gameContext);
            } else if (p_operation.equalsIgnoreCase("remove") && p_argument.split(" ").length == 1) {
                p_mapToBeUpdated.removeCountry(p_argument.split(" ")[0]);
                this.setD_MapServiceLog("Country " + p_argument.split(" ")[0] + " removed successfully!",
                        p_gameContext);
            } else {
     ...
     * @throws InvalidMap invalidmap exception
     */
    public Map addRemoveNeighbour(GameContext p_gameContext, Map p_mapToBeUpdated, String p_operation,
            String p_argument)
            throws InvalidMap {
        ...
        try {
            if (p_operation.equalsIgnoreCase("add") && p_argument.split(" ").length == 2) {
                ...
                this.setD_MapServiceLog("Neighbour Pair " + p_argument.split(" ")[0] + " " + p_argument.split(" ")[1]
                        + " added successfully!", p_gameContext);
                ...
                this.setD_MapServiceLog("Neighbour Pair " + p_argument.split(" ")[0] + " " + p_argument.split(" ")[1]
                        + " removed successfully!", p_gameContext);
            } else {
     ...
     * @return true/false based on the successful save operation of the map to a
     *         file
     * @throws InvalidMap InvalidMap exception
     */
    public boolean saveMap(GameContext p_gameContext, String p_fileName) throws InvalidMap {
        try {
            ...
            if (!p_fileName.equalsIgnoreCase(p_gameContext.getD_map().getD_mapFile())) {
                p_gameContext.setD_error("Kindly provide the same file name to save which you have given for edit");
                return false;
            } else {
                if (null != p_gameContext.getD_map()) {
     ...
     * @throws IOException Exception
     */
    private void parseMapToFile(GameContext p_gameContext, FileWriter l_writer, String l_mapFormat) throws IOException {
