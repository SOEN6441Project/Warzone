...
public class ConquestMapFileReader implements Serializable {
        ...
        switch (p_switchParameter) {
            ...
            case "country":
                List<String> l_countryLines = p_fileLines
                        .subList(p_fileLines.indexOf(Constants.CONQUEST_TERRITORIES) + 1,
                                p_fileLines.size());
    ...
    /**
     * The parseContinentsMetaData method parses the extracted continent data from
     * the map file.
     * 
    ...
    /**
     * The parseCountriesMetaData method parses the extracted country and border
     * data from the map file.
     * 
     ...
     * 
     * @param p_continentList List of continents from which filtering has to be
     *                        done.
