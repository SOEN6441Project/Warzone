...
public class TournamentModeView {
  ...
  private void renderMapName(Integer gameIndex, String mapName) {
    String formattedString =
        String.format("%s %s %d %s", mapName, " (Game Number: ", gameIndex, " )");
    renderSeparator();
  ...
  private void renderGames(GameContext gameState) {
    ...
    if (gameState.getD_tournamentWinner() == null) {
      ...
      conclusion =
          "Winning Player Strategy: "
              + gameState.getD_tournamentWinner().getD_playerBehaviorStrategy();
    }
    ...
    for (int i = 0; i < gameState.getD_playersFailed().size(); i++) {
      ...
      if (i < gameState.getD_playersFailed().size() - 1) commaSeparatedPlayers.append(", ");
    }
    String losingPlayers =
        "Losing Players -> "
            + WordWrap.from(commaSeparatedPlayers.toString())
                .maxWidth(Constants.CONSOLE_WIDTH)
                .wrap();
