...
public class TournamentTest {
    ...
    public void shouldHandleInvalidMapArgs() throws InvalidMap, InvalidCommand {
        ...
        assertFalse(l_tournament.parseTournamentCommand(d_gameState, "M",
                "test.map test123.map canada.map conquest.map swiss.map europe.map", new GameContext()));
    }
    /**
    ...
    public void shouldPlayValidTournament() throws InvalidMap, InvalidCommand {
        StartupPhase l_startUpPhase = new StartupPhase(new GameContext(), d_gameState);
        Tournament l_tournament = new Tournament();
        GameContext l_gameEngine = new GameContext();
